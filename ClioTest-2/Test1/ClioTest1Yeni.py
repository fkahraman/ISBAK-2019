...
def toplam(sensor, min):

    rangeData = len(sensor)

    for i in range(rangeData):

        araDeger = int(sensor[i]) - int(min[i])
        minFarkToplam.append(araDeger)

    DataSetToplam = list(zip(minFarkToplam))
    df = pd.DataFrame(data=DataSetToplam, columns=None)
    df.to_csv('aratoplamDegerleri.txt', index=False, header=False)
...
def fark(data):
    ...
    #Location2 = r'C:\Users\fatih\PycharmProjects\untitled\AFTER ISBAK 2019\ClioTestYeni\Test1\farkDegerleri.txt'
